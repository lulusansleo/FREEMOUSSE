...
FFT::FFT() : m_impl(std::make_unique<Impl>())
{
    ...
    d.out = static_cast<fftwf_complex*>(
        fftwf_malloc(sizeof(fftwf_complex) * kSpectrumBins));
    if (!d.out) throw std::runtime_error("FFTW3: output buffer allocation failed");

    // FFTW_MEASURE: expensive one-time planning for fastest execution
    d.plan = fftwf_plan_dft_r2c_1d(
        kFFTSize, d.in.data(), d.out, FFTW_MEASURE);
...
FFT::~FFT()
{
    if (m_impl->plan) fftwf_destroy_plan(m_impl->plan);
    if (m_impl->out)  fftwf_free(m_impl->out);
}
